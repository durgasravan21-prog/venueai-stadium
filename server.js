...
function simulateMatch() {
  if (matchState.status === 'pre_match') return;
  if (matchState.status === 'post_match') return;

  if (matchState.status === 'first_half') {
    matchState.minute++;
    if (matchState.minute >= 45) {
      matchState.status = 'halftime';
      addAlert('info', 'Halftime reached. Expecting concession rush.', 'system');
    }
    // Random goal chance
    if (Math.random() < 0.008) {
      ...
      addAlert('success', `⚽ GOAL! ${team} scores at ${matchState.minute}'!`, 'match');
    }
  } else if (matchState.status === 'halftime') {
    // Halftime lasts ~15 seconds in simulation (represents 15 min)
    setTimeout(() => {
    ...
    if (matchState.minute >= 90) {
      ...
      addAlert('warning', 'Match ended. Initiating crowd exit management.', 'system');
    }
    if (Math.random() < 0.008) {
      const isHome = Math.random() > 0.45;
      if (isHome) matchState.homeScore++;
  ...
  switch (action) {
    ...
    case 'reset':
      matchState = {
        ...
        homeTeam: 'Metro United', awayTeam: 'City Strikers',
